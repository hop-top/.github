...
goreleaser's `brews.repository.name` field controls this; set it
to `homebrew-tap`. Same for `scoops.bucket.name`.
...
| Created `homebrew-<binary>` or `scoop-<binary>` tap/bucket repo per binary | Misread convention — taps are org-wide, not per-binary | Use `<org>/homebrew-tap` + `<org>/scoop-bucket` (single repos serving every org binary). Delete the per-binary tap; point goreleaser's `brews.repository.name` at `homebrew-tap`. See [Org-wide tap/bucket convention](#org-wide-tapbucket-convention). |
