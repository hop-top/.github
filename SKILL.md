...
commits → release-please opens standing PR
          ...
          release-please creates tag <component>/v<version>
            ↓ tag push
          your publish.yml triggers       (+ optional goreleaser.yml)
            ↓ uses:                          ↓ uses:
          publish-on-tag.yml               goreleaser-on-tag.yml
            ↓ dispatches                     ↓
          publish-{ts,py,rs}.yml +         cross-platform binaries
            mirror-subtree.yml             + Homebrew formula push
            ↓                                ↓
          registry + mirror push           GitHub Release assets + tap
```

`hop-top/.github` owns the **publish/mirror/binaries** half.
release-please (consumer-side, configured in YOUR repo) owns the
**version/tag** half. Both compose; you wire them up. The
goreleaser-on-tag lane is opt-in — only Go CLIs that ship
distributable binaries need it (see
[Shipping Go binaries with GoReleaser](#shipping-go-binaries-with-goreleaser)).
...
## Shipping Go binaries with GoReleaser

`publish-on-tag.yml`'s `go` ecosystem is a no-op — Go modules ship
via `proxy.golang.org` pulling git tags. If you want cross-platform
**binaries** (linux / macOS / windows × amd64 / arm64) as GitHub
Release assets, plus a Homebrew formula, use the dedicated
[`goreleaser-on-tag.yml`](.github/workflows/goreleaser-on-tag.yml)
reusable workflow.

### Why a separate workflow

GoReleaser's `monorepo:` block (which strips the
`<component>/` tag prefix so `{{.Version}}` resolves cleanly) is
**[GoReleaser Pro][gr-monorepo]** only. The OSS version sees
`<component>/v<version>` tags as opaque strings, breaking
artifact names, ldflag injection, and the Homebrew formula's
`version:` field.

`goreleaser-on-tag.yml` works around this by deriving the bare
semver in shell and exposing it as `BARE_VERSION` (plus the
literal `RELEASE_TAG`) to the GoReleaser step's environment.
Both `<component>/v<version>` and plain `v<version>` shapes
work — same workflow, no caller config to tune.
...
release-bot App token mint, and the GoReleaser invocation.

### `.goreleaser.yaml` template

In your `.goreleaser.yaml`, reference `{{ .Env.BARE_VERSION }}`
and `{{ .Env.RELEASE_TAG }}` wherever you'd normally use
`{{.Version}}` or `{{.Tag}}`. This is the only Go template-side
adjustment needed:
...
attach their artifacts to it (GoReleaser uses `mode: append`).

### Requirements

- The `release-bot` GitHub App must be installed on the source
  repo **and** the Homebrew tap repo (if used) — same install,
  scoped to both.
