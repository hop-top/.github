...
| Fixed `publish.yml` on main, retry still fails | `publish.yml` snapshots from the tag's commit; `gh run rerun` reuses the original workflow refs (same for `@v0`/`@main` reusable-workflow refs) | Delete + recreate the tag at current main, OR cut a new patch tag with the fix. See [SKILL.md § Re-runs use the tag's workflow snapshot, not main](../../SKILL.md#re-runs-use-the-tags-workflow-snapshot-not-main), [concepts/mental-model.md § Snapshot semantics](../concepts/mental-model.md#snapshot-semantics), and [how-to/retrigger-failed-publish.md](../how-to/retrigger-failed-publish.md). |
| Umbrella / meta-component tag (`<umbrella>/vX.Y.Z`) finishes green but every publish job shows `skipped` | release-please `.` root entry (e.g. `poly-cite`, `poly-uri`, `poly-kit`) has no matching `ecosystems` map entry; `parse` resolves no component and emits a `::notice::` with empty outputs, gating every downstream job | This is the intended contract — do NOT add a stub `ecosystems` entry for the umbrella. See [SKILL.md § Umbrella / meta-component tags](../../SKILL.md#umbrella--meta-component-tags). |
...
| Build step fails with `ERR_PNPM_SPEC_NOT_SUPPORTED_BY_ANY_RESOLVER "&&"` | `run: $CMD` doesn't re-parse shell operators in env-var commands (tracked in [#9](https://github.com/hop-top/.github/issues/9)); resolved for both test and build at `publish-{py,rs,ts}.yml@v0.4.3+` | Pin `@v0` or `@v1`. On an older pin, move the pipeline into a package.json script (`ci:build`) so the workflow command is single-token. [Details](../../docs/failure-modes.md#err_pnpm_spec_not_supported_by_any_resolver-on-build-step) |
| `ts` test step fails `vitest: not found` after setting `test-command` | The override replaced the default's implicit `pnpm install` | Keep the install in the override or use `pnpm dlx … vitest run` + a `ci:build` script. See [troubleshooting/ts.md § Overriding `test-command`](ts.md#overriding-test-command-drops-the-implicit-install). |
...
| Mirror push fails `GH013: Repository rule violations ... Push cannot contain secrets` | The mirror repo's push protection scans the **full subtree history**; realistic-looking fixture secrets (e.g. a redaction test's sample Slack token) in ANY historical commit block the push — even when current files are clean and the monorepo is public | Click the `unblock-secret` URL from the failed run's log (per mirror repo, one-time per secret), then `gh run rerun <id> --failed` — rerun is fine here since no workflow changed. Each language mirror blocks independently on its own fixture. |
| Standing release PR re-lists commits the previous release already shipped (changelog double-count) | Under `skip-github-release`, release-please anchors commit ranges on the latest GitHub **Release object** and cannot see companion-created tag-only releases | Versions/tags stay correct; don't merge the PR without checking its changelog against the last shipped release. See [how-to/release-free-monorepo.md § Gotchas](../how-to/release-free-monorepo.md#gotchas). |
...
| Unknown component `<name>` fires for a `release-type: simple` root/aggregate package with no registry target | `bootstrap-checklist.md` says avoid a root `.` package, but if you have one anyway (e.g. version-bumping an umbrella release across all components), its tag still matches `publish.yml`'s `tags: ['*/v*']` trigger and the reusable workflow hard-fails since it's not in `ecosystems:` | No longer fails: `publish-on-tag` now skips components missing from `ecosystems:` with a `::notice::` and green-skipped jobs (see the umbrella row above). The old workaround — excluding the tag from the trigger (`tags: ['*/v*', '!<root-component>/v*']`) — is optional and harmless to keep. Still don't add the root to `ecosystems:`; it has nothing to publish. |
