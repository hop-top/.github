...
| `RELEASE_BOT_APP_ID` | release-please job + `release-tag-on-merge` (both mint via `actions/create-github-app-token@v1`) + `spec-status-line` (mints via `@v3`; the token pushes the status-line commit) | Org | GitHub App ID for the hop-top release-bot. Paired with `RELEASE_BOT_PRIVATE_KEY`. See [GitHub App permissions](#github-app-permissions). |
| `RELEASE_BOT_PRIVATE_KEY` | release-please job + `release-tag-on-merge` (both mint via `actions/create-github-app-token@v1`) + `spec-status-line` (mints via `@v3`) | Org | GitHub App private key. Paired with `RELEASE_BOT_APP_ID`. |
